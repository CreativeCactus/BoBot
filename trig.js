module.exports=(self)=>{
    ...
    (user, userID, channelID, message, event, ctx)=>{ //Is masters message
        ctx.Master=(userID==self.MasterID)
        return ctx
    },
    (user, userID, channelID, message, event, ctx)=>{ //set TTS
    ...
    (user, userID, channelID, message, event, ctx)=>{ //eval bash
        var rx=( /^bash `((.|\n){2,})`/gi ), act = ""
    ...
    (user, userID, channelID, message, event, ctx)=>{ //Sandbox js
        var rx=( /^js `((.|\n){2,})`/gi ), act = ""
    ...
    (user, userID, channelID, message, event, ctx)=>{ //Native eval
        var rx=( /^jsunsafe `((.|\n){2,})`/gi ), act = ""
        if ( !message.match(rx) ) return
    ...
    (user, userID, channelID, message, event, ctx)=>{ //allow setting a 'joke' context    
    ...
    (user, userID, channelID, message, event, ctx)=>{ //behaviours within a joke context
    ...
    (user, userID, channelID, message, event, ctx)=>{ //redmine
